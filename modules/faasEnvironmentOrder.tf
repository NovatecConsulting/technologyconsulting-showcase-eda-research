resource "azurerm_app_service_plan" "function_app_service_plan_linux" {
   name = "tc-eda-iac-${var.environment}-function-app-service-plan-linux"
...
resource "azurerm_function_app" "order_function_producer" {
    ...
    location = azurerm_resource_group.resourceGroup.location
    resource_group_name = azurerm_resource_group.resourceGroup.name
    app_service_plan_id = azurerm_app_service_plan.function_app_service_plan_linux.id
